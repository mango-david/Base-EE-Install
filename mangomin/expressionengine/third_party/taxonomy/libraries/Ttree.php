...
class Ttree {
	...
	function build_cp_list($array, $ind = '')
	{
    	...
    	foreach($array as $data)
    	{
    		...
    		$status_text		= ($data['status'] && $data['status'] != 'open') ? ' + '.ucfirst($data['status']) : '';
    		...
    		$entry_date 		= (isset($data['entry_date'])) ? $data['entry_date'] : 0;
        	...
        	$str .= $ind."		$status $date_indicator<a href='$this->_base_url&amp;method=manage_node&amp;node_id=$node_id&amp;tree_id=$tree_id'>$label</a>\n"; 
	...
	function build_list($array, $tagdata, $options)
	{
		...
		$options['timestamp'] 		= ($options['timestamp']) ? $options['timestamp'] : $this->EE->localize->now;
		$options['show_future_entries']= ($options['show_future_entries']) ? $options['show_future_entries'] : 'no';
		$options['show_expired'] 	= ($options['show_expired']) ? $options['show_expired'] : 'no';
		$options['li_has_children_class'] = ($options['li_has_children_class']) ? $options['li_has_children_class'] : 'has_children';
		$options['disable_active_styles']= ($options['disable_active_styles']) ? $options['disable_active_styles'] : 'no';
	    ...
	    {    
	    	...
	    	$data['expiration_date'] = (isset($data['expiration_date'])) ? $data['expiration_date'] : 0;
	    	...
	    	if(
	    		...
	    		( 
	    		 ...
	    		 &&
	    		( 
		    		$data['entry_date'] < $options['timestamp'] || $options['show_future_entries'] == "yes" // future entries?
		    	)
		    	&&
	    		( 
		    		$data['expiration_date'] == 0 || $data['expiration_date'] > $options['timestamp'] || $options['show_expired'] == "yes" // expired?
		    	)
	    	  ...
	    	  )
	    	{
		    	...
		    	if($options['path'])
				{
					foreach($options['path'] as $parent_node)
					{
						...
						{
							$active_parent = ($options['disable_active_styles'] != 'yes') ? 'active_parent' : '';
							// Added by @nevsie
				...
				{
					$active = ($options['disable_active_styles'] != 'yes') ? $options['node_active_class'] : '';
					// Added by @nevsie
	    		...
	    		else
	    		{
						...
						)
					{
						...
						$node_url 	= rtrim(reduce_double_slashes($node_url), '/');
						$viewed_url = rtrim(reduce_double_slashes($viewed_url), '/');
						
						if($node_url === $viewed_url && $options['disable_active_styles'] != 'yes')
						{
						...
						{
							$children = 'yes';
							$children_class = $options['li_has_children_class'];
						}    		
						...
						$variables = array(
											...
											'node_url' => $node_url,
											'node_relative_url' => str_replace( $this->EE->functions->fetch_site_index() , '' , $node_url),
											'node_active' => $active,
											...
											'node_entry_expiration_date' => $data['expiration_date'],
						...
						if (isset($options['li_css_class:level_'.$data['level']]))
						{
							$li_css_class = $options['li_css_class:level_'.$data['level']];
						}

						// build our node class and remove any extra spaces
						$node_class = preg_replace('/\s\s+/', ' ', "node_$unique_class level_$level $children_class $active_parent $active $first_class $last_class $li_css_class");
						...
						if(isset($data['children']) && $data['level'] < $options['depth'])
						{
							...
							if (isset($options['ul_css_id:level_'.$data['level']]))
							{
								$options['ul_css_id'] = $options['ul_css_id:level_'.$data['level']];
							}
							
							if (isset($options['ul_css_class:level_'.$data['level']]))
							{
								$options['ul_css_class'] = $options['ul_css_class:level_'.$data['level']];
							}
    ...
    /**
     * @fccotech: added this function to search entry by custom_url / uri
     * @return boolean 
     */
    function get_node_id_from_url_override()
	{
		$uri = $this->EE->uri->uri_string();
		
		$this->EE->db->where('custom_url',$uri);
		$this->EE->db->or_where('custom_url','/'.$uri);
		$this->EE->db->limit(1); // we only want the first of all parents
		$query = $this->EE->db->get($this->tree_table);
	...
	* 
	*/
    function stash_node_urls_array($tree_id, $hide_dt_group)
    {
	
		$nodes_array = $this->node_urls_array();	
		...
		foreach($nodes_array as $node)
		{
			...
			if($node['custom_url'] == "")
			{
				...
				$entries[ 'node_id:'.$node['node_id'] ] = $this->EE->functions->fetch_site_index().$template_group.'/'.$node['template_name'].'/'.$node['url_title'];
			}
			else
			{
				...
				{
					$entries[ 'node_id:'.$node['node_id'] ] =  $this->entry_id_to_page_uri($node['entry_id']);
				}
				// relative anchor perhaps...
				elseif($node['custom_url'][0] == "#")
    			{
    				$entries[ 'node_id:'.$node['node_id'] ] = $node['custom_url'];
    			}
    			else
    			{
    				// external link or relative?
    				$entries[ 'node_id:'.$node['node_id'] ] = ((substr(ltrim($node['custom_url']), 0, 7) != 'http://') 
    											  && (substr(ltrim($node['custom_url']), 0, 8) != 'https://') 
    											  ?  $this->EE->functions->fetch_site_index() : '') . $node['custom_url'];
    			}
			}
			
			$entries[ 'node_id:'.$node['node_id'] ] = reduce_double_slashes( rtrim($entries[ 'node_id:'.$node['node_id'] ], '/') );
			
			if($node['entry_id'])
			{
				$entries[ 'entry_id:'.$node['entry_id'] ] = $entries[ 'node_id:'.$node['node_id'] ];
				$entries[ $node['entry_id'] ] = $entries[ 'node_id:'.$node['node_id'] ]; // keep this incase other devs have used it
			}
		}
