...
class Taxonomy_ext {
	...
	public function sessions_end($session)
	{
		...
		// and evaluate the performance difference. 
		// Taxonomy will query and load what it needs if it hasn't been loaded here.
	...
	/**
	 * Adds our Taxonomy menu to the main CP nav (replaces accessory as of 2.2)
	 *
	 * @param 
	 * @return 
	 */
	...
	public function cp_menu_array($menu)
	{
		...
		if(
			($this->EE->config->item('disable_taxonomy_cp_nav'))
			OR 
			( ! $this->EE->session->userdata('assigned_modules') || ! $this->EE->cp->allowed_group('can_access_addons', 'can_access_modules')) 
			&& 
		...
		if ($this->EE->session->userdata('group_id') != 1)
		{
			...
			$taxonomy_module_id  = $this->EE->db->select('module_id')
								  ->where('module_name', 'Taxonomy')
								  ->get('modules')
								  ->row('module_id');
			
			
			if ( ! isset($this->EE->session->userdata['assigned_modules'][$taxonomy_module_id]) OR  $this->EE->session->userdata['assigned_modules'][$taxonomy_module_id] !== TRUE)
			{
				return $menu;
			}
		}
		...
		return $this->array_insert($menu, $t_menu, 1);
	}
